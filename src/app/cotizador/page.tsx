...
function Editor({
c, onChange, onSave, onBack, onPrint, onShare, onExportPremiumPDF, saved,
}: {
c: Cotizacion;
...
{premiumMode ? (
<button
onClick={onExportPremiumPDF}
className="px-3 py-1.5 text-xs font-mono border border-[#2563eb]/40 rounded-lg text-[#2563eb] hover:bg-[#2563eb]/10 transition-colors"
>
↓ PDF Premium
</button>
) : (
...
{saved ? "✓ Guardado" : "Guardar"}
    ...
    {/* Nota final */}
    <div className="border border-white/5 bg-surface rounded-xl p-4">
      <label className={labelCls}>Nota final / descripción del servicio</label>
      <textarea
        rows={3}
        className={inputCls + " resize-none"}
        value={c.notaFinal}
        onChange={e => set({ notaFinal: e.target.value })}
        placeholder="Descripción del servicio o mensaje al cliente..."
      />
      <p className="text-[9px] font-mono text-white/20 mt-1.5">
        Este texto aparece como descripción del servicio en la cotización premium.
      </p>
    </div>
  </div>
  ...
  <div className="sticky top-24">
    <div className="flex items-center justify-between mb-2">
      <div className="text-[10px] font-mono tracking-wider uppercase text-white/20">Vista previa</div>
      {premiumMode && (
        <div className="text-[9px] font-mono text-[#2563eb] border border-[#2563eb]/30 px-2 py-0.5 rounded bg-[#2563eb]/5">
          Modo Premium · Fondo blanco
        </div>
      )}
    </div>

    {premiumMode ? (
      // Premium: escalar para caber en la columna
      <div
        style={{
          transform: "scale(0.68)",
          transformOrigin: "top left",
          width: 794,
          // compensar el espacio vacío por el scale
          marginBottom: `calc((794px * 0.68 - 794px) + 1123px * 0.68 - 1123px)`,
        }}
      >
        <QuoteTemplatePremium c={c} />
      </div>
    ) : (
      <Preview c={c} />
    )}
        ...
        {view === "editor" && current && (
          <>
            ...
            <Editor
              ...
              onShare={handleShare}
              onExportPremiumPDF={handleExportPremiumPDF}
              saved={saved}
            />
          </>
        )}
